...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function renderTable(items) {
        for (const row of items)tbody.append(addRow(row.name, row.position, row.office, row.age, row.salary));
    }
  ...
  `;
    ...
    function saveButton() {
        ...
        if (validForm(name, position, office, age, salary)) {
            tbody.appendChild(addRow(name, position, office, age, `$${parseInt(salary).toLocaleString("en-US")}`));
